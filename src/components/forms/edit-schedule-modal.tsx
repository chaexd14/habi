...
import { updateScheduleApi } from "@/lib/api/schedule";
import { useSchedule } from "@/providers/schedule-provider";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { DatePicker } from "@/components/ui/date-picker";
import { X, Loader2, Clock, CalendarDays, CheckCircle2, Pencil } from "lucide-react";
...
function calculateDiffDays(startIso?: string | null, endIso?: string | null): number {
...
export function EditScheduleModal({
  ...
  useEffect(() => {
    if (schedule) {
      ...
      const s = schedule.start_date || "";
      const e = schedule.end_date || "";
      setStartDate(s);
      setEndDate(e);

      if (s && e) {
        const diff = calculateDiffDays(s, e);
        if (diff === 3) {
          setDurationOption("3");
        } else if (diff === 7) {
          setDurationOption("7");
        } else if (diff === 14) {
          setDurationOption("14");
        } else {
          setDurationOption("custom_range");
          setCustomDaysCount(diff > 0 ? String(diff) : "30");
        }
      } else {
  ...
  const handleCustomDaysChange = (daysVal: string) => {
    setCustomDaysCount(daysVal);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await updateScheduleApi(schedule.id, {
        title: title.trim(),
        description: description.trim() || null,
        start_date: durationOption === "ongoing" ? null : (startDate || null),
        end_date: durationOption === "ongoing" ? null : (endDate || null),
      });
  ...
  return createPortal(
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="edit_sched_modal_title"
    ...
    >
      ...
      <div className="relative w-full max-w-lg rounded-xl border border-border bg-card text-card-foreground p-5 shadow-lg z-10 max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-98 duration-150 space-y-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 id="edit_sched_modal_title" className="text-sm font-semibold text-foreground">
              Edit Schedule
            </h3>
            <p className="text-xs text-muted-foreground">
              Update schedule details, timeline, and recurring settings.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-left">
          <div className="space-y-1">
            <label htmlFor="edit_sched_title_input" className="text-xs font-medium text-foreground">
              Schedule Title <span className="text-destructive">*</span>
            </label>
            <Input
              id="edit_sched_title_input"
              type="text"
          ...
          <div className="space-y-1">
            <label htmlFor="edit_sched_desc_input" className="text-xs font-medium text-foreground">
              Description (Optional)
            </label>
            <Input
              id="edit_sched_desc_input"
              type="text"
          ...
          <div className="space-y-2 pt-1 border-t border-border">
            ...
            {durationOption === "custom_days" && (
              <div className="flex items-center gap-2 pt-1">
                <label htmlFor="edit_custom_days_input" className="text-xs text-muted-foreground shrink-0">
                  Number of days:
                </label>
                <Input
                  id="edit_custom_days_input"
                  type="number"
                  min={1}
                  max={365}
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div className="space-y-1">
              <label htmlFor="edit_sched_start_date" className="text-xs font-medium text-foreground">
                Start Date
              </label>
              <DatePicker
                id="edit_sched_start_date"
                date={startDate}
                onDateChange={(d) => {
                  setStartDate(d);
                  if (durationOption !== "custom_range" && durationOption !== "ongoing") {
                    const days = durationOption === "3" ? 3 : durationOption === "7" ? 7 : durationOption === "14" ? 14 : parseInt(customDaysCount, 10) || 30;
                    setEndDate(computeEndDate(d, days));
                  }
                }}
            ...
            <div className="space-y-1">
              <label htmlFor="edit_sched_end_date" className="text-xs font-medium text-foreground flex items-center justify-between">
                <span>End Date</span>
